...
\framed{
{\Large\centering\it
Syb’hérá srá dédv́ér dý\;! evaú lnérd s’aúłau’r\;! \\
— Ráhb’hes Ihic’hlávê c’hónývâ sýrá dý y’íb’hâ.
\par}
...
\s{pres.ant.3n.pass}-pour-\s{circ}|\s{acc.indef}-month|fucking|two|\s{voc.pl.2pron}|\s{abs.pl}-nerd|it’s.about.time.that

ráh-b’hes|i\Sl hic’hlávê|c’hónývâ|sý-rá|dý|y’íb’h-â
\s{insv}-16.july|\s{acc}\Sl 16.february|famously|\s{gen.pl.indef}-month|two|follow-\s{ptcp}
}
}
