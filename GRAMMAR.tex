...
in PF *\w{ces}, e.g. \w{s’ḍalẹ} ‘the tables (\s{acc})’ (not \w{s’thalẹ}, which is the singular), as well as in
...
accusative, are borrowed from demonstratives instead (\s{def} from PF *\pf{celui} and \s{indef} from PF \pf{ce}); the definite
...
\noindent The diachrony of these forms is likely from subjunctive constructions with \s{pf} *\pf{que} in the active
