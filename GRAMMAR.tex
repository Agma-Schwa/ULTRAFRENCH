...
The \s{ess} case is used to mean ‘as X’, ‘like X’, ‘in the form of X’, or ‘in the role of X’, e.g. \w{lẹt’hiy’e dyt’halẹ ḅárýny’éjúré}
...
The \s{abess} case means ‘without’, e.g. \w{sá-árb} ‘without the tree’.

\subsubsection{Inessive}
The \s{iness} indicates that an object is inside another, e.g. \w{dwávnásḍér} ‘in the castle’. For objects which you tend
...
The \s{contr} case has the meaning of ‘against’, e.g. \w{n c’hauvẹ} ‘hate against me’.
...
\subsubsection{Agent and Patient Nouns}% agent noun, patient noun
...
where applicable, e.g. present \w{ad’hórâ} ‘loving’ and \w{vvaúríhê} ‘remembering’ become \w{ad’hórêr} ‘having loved’,
and \w{vvaúríhêr} ‘having remembered’. This is one of the rare cases where the quality of the desinence overrides that of
the stem.

In both tenses, the suffixes coalesce with vowels of the same quality before them, nasalising them if
they are already nasalised, e.g. \w{jvvaúríhé} ‘I remembered’. If the verb form ends with a \textit{nasal} vowel,
...
\textit{ệ}. If the preceding vowel differs in quality, hiatus is maintained, e.g. \w{ní} becomes \w{âníér} ‘having
been denied’. If the final stem vowel is \w{ẹ}, it is deleted, and the suffix is simply appended, e.g. \w{jrévôt’hé}
‘I returned’ from \w{révôt’hẹ} ‘return’.
...
\subsection{Fully-Glossed Examples}
...
\subsubsection{Two Stanzas from ‘The Rime of the Ancient Mariner’}
...
\subsubsection{The Tragedy of Darth Plagueis the Wise}
...
\subsection{Unglossed Translations}
\subsubsection{The Misanthrope – \s{Molière}}
\begin{verse}\it
