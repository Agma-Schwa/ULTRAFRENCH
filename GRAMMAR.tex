...
\cs_new:Npn \__gloss_five_lines:n #1 {
    \cs_gset:Npn \__formatter_i:n ##1 { \multicolumn {100} {@{}l} {\itshape\bfseries##1} }
    ...
    \cs_gset:Npn \__formatter_v:n ##1 { \multicolumn {100} {@{}l} {‘##1’} }
...
⟨y’⟩ (or sometimes just ⟨’⟩, especially word-initially) and ⟨ý’⟩. The ʁ-fricated fricatives /βʶ, ɮ̃ʶ, sʶ, ɕʶ, zʶ, ʑʶ/
...
{\itshape
Słérá de c’hóný áb’hás’ô, ráy’ê y’aúhý dís dyb’hóy’e sab’héy’. Ez lé-el lalebet’he z’ihór bet’hê rêsol daudé.

}

\gloss {
    słé-rá| ḍẹ| c’hóný| áb’hás’ô| ráy’ê| y’aúhý| ḍ-ís
    \s{cons.pl.def}-law|all|well.known|\s{gen.sg.def}\textbackslash aviation|way|there.is.no|\s{inf.act-subj}\textbackslash can
}

\gloss {
    dy-b’hóy’ẹ | s-ab’héy’ |ez |lé-el |z’| ihór
    \s{part.sg.def}-to.fly|\s{acc.sg.indef}-bee|its|\s{nom.pl.def}-wing|its|\s{acc.sg.def}\textbackslash body
}

\gloss {
    bet’hê | rê-sol | ḍ-auḍé
    be.small\textbackslash\s{part.act}|\s{abl.sg.def}-soil|\s{inf.act}-obtain
}
