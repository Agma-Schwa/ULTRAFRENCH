...
The \s{abs} and \s{voc} can’t be stacked since they are mostly unmarked. While this is sometimes done humorously, such
interpretations are generally purposefully ungrammatical. In contexts where a stacked \s{abs} would be required, an
explicit demonstrative, such as \w{swi} ‘the one’, is used instead. The only exception to this is the \s{def abs pl},
which always has an overt realisation and can thus be stacked, but this is exceedingly rare.
