...
The term ACI is Latin for *accūsātīvus cum īnfīnītīvō* ‘accusative with infinitive’. As the name would suggest, this
grammatical construction consists of a dependent clause formed by an accusative noun together with an infinitive; the
...
the ACI *sbhaú àfér* ‘a bridge to be built’. Since ‘a bridge’ is the object in this case, the passive infinitive
is used. Observe how this sentence’s translation also uses an ACI with a passive infinitive in both English (‘Charles
...
ACIs in these cases. Just how multiple ‘that’ clauses can be chained in English, so can multiple ACIs in UF.
\gloss {
...
When multiple ACIs are chained together, they are nested such that accusative comes first and the infinitive
last or vice versa, and any nested ACIs are placed inbetween; observe that, in the sentence above, the ACI
...
Furthermore, note that the finite verb of the matrix clause of an ACI receives only a subject marker if the
ACI is the object and vice versa. Thus, we have *jsav́á* ‘I know’ in the example above instead of e.g.
...
In addition to ACIs, UF also has PCIs, which use the *partitive* case instead. The partitive generally indicates
that an action is incomplete, and thus PCIs can be used to express something similar; for instance:
\gloss {
...
ACIs and PCIs much more prolifically than English does. A somewhat literal translation of this sentence would be
...
Finally, note that ‘that’ is not always expressed with an ACI or PCI. Certain verbs, e.g. verbs of fearing, may
...
\footnotetext{UF does not use the letters *p* and *q*, and thus, discussions of propositional logic in UF tend to
...
{\itshape\bfseries
Rub’hráy’ó rát’he au sré au sfèhe laut’hâ adŷbáłýr Áraúl dedesle, s aút’hiy’ey’ó sývéhýr dýhisdé sérdé laúây’êr;
aúc’haúbrâdy’ó’sý’ýâ vé dúr dyhaúbhausy’ô sehabhvísy’ô. Sýlývy’ér saúr c’hesse? Lec’hdraúv\-nét’hic’hâ nérje c’hesse?
...
\gloss {
    aḍŷ-ḅáłýr|á-raúl|dẹ-deslẹ|s|aú-t’hiy’e-y’ó|sý-véhýr
    \s{interess.pl.indef}-speaker|\s{gen}-language|\s{inf}-detect|if|\s{1pl}-use-\s{1pl}|\s{gen.pl.indef}-measure
}
...
\medskip\noindent
Literal translation: We can, you see, detect both rays and beams of energy floating between speakers of The Language (= UF)
...
speaker uses a PCI (*dybháł*) instead of an ACI (*ibháł*) for ‘speaking’ here; had they used an ACI, the meaning would be closer to ‘the act
