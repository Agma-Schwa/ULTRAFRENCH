...
\def\glossaryformat{\s}
\RenewDocumentEnvironment {glossary} {
    m      %% Columns.
    m      %% Term width.
    O{1em} %% Column sep.
} {
    \begingroup
        \def \g ##1 ##2 {
            \noindent \leavevmode \hbox {
                \hbox to 4em {\glossaryformat{##1}\hfil}
                \enskip
                ##2\hfil
            } \par
        }
...
\def \footnoterule {
    \medskip %% To prevent the rule from colliding with text.
    \kern -3\p@
...
\cs_new:Npn \examples {
    \ifvmode\else\unskip\par\fi
    \vskip 3pt plus 3pt \relax
    \penalty -500 \relax %% Before examples is a good place to break.
    \begingroup
        \def \labelitemi { \(\diamond\) }
        \let \olddoubleslash \\
        \def \\ { \olddoubleslash \llap } %% Move '‘' of translation into the indentation margin.
        \items
}
...
\cs_new:Npn \endexamples {
        \enditems
    \endgroup
    \penalty -500 \relax %% After examples is a good place to break.
    \vskip 3pt plus 3pt \relax
    \__uf_no_indent:
}
...
%% No interrobang in Minion 3...
\catcode`‽=13
\def‽{{\ifvmode\leavevmode\fi\rlap{?}\kern.5pt!}}
...
\chapter*{Glossary}
This is a glossary of general grammatical abbreviations used throughout the grammar as well as the dictionary. The latter
also has an extended set of abbreviations in italics; for those, see Chapter~\ref{chap:dictionary}.
\begin{glossary}{3}{4em}
    \g{abess}{abessive}
    \g{abl}{ablative}
    \g{abs}{absolutive}
    \g{acc}{accusative}
    \g{aci}{see §~\ref{subsec:aci-pci}}
    \g{adess}{adessive}
    \g{aff}{affirmative}
    \g{all}{allative}
    \g{antabl}{antablative}
    \g{antall}{antallative}
    \g{antess}{antessive}
    \g{caus}{causal}
    \g{circ}{circumfix}
    \g{circabl}{circumablative}
    \g{circess}{circumessive}
    \g{circlat}{circumlative}
    \g{citr}{citressive}
    \g{com}{comitative}
    \g{comp}{comparative}
    \g{cond}{conditional}
    \g{cons}{considerative}
    \g{contr}{contrative}
    \g{contress}{contressive}
    \g{dat}{dative}
    \g{def}{definite}
    \g{del}{delative}
    \g{den}{denying}
    \g{distr}{distributive}
    \g{ela}{elative}
    \g{ess}{essive}
    \g{exess}{exessive}
    \g{fut}{future}
    ...
    \g{iness}{inessive}
    \g{inf}{infinitive}
    \g{instr}{instrumental}
    \g{insv}{instructive}
    \g{intress}{interessive}
    \g{nd}{northern dialect}
    \g{nom}{nominative}
    \g{opt}{optative}
    \g{orient}{orientative}
    \g{part}{partitive}
    \g{pass}{passive}
    \g{pci}{see §~\ref{subsec:aci-pci}}
    \g{perl}{perlative}
    \g{pl}{plural}
    \g{pres ant}{present anterior}
    \g{pres}{present}
    \g{pret}{preterite}
    \g{pstela}{postela}
    \g{pstess}{postessive}
    \g{pstlat}{postlative}
    \g{ptcp}{participle}
    \g{sd}{standard dialect}
    \g{sg}{singular}
    \g{spress}{superessive}
    \g{sprlat}{superlative}
    \g{subess}{subessive}
    \g{subj}{subjunctive}
    \g{subl}{sublative}
    \g{subst}{substitutive}
    \g{superl}{superlative}
    \g{surl}{surlative}
    \g{transall}{transallative}
    \g{transess}{transessive}
    \g{transl}{translative}
    \g{voc}{vocative}
\end{glossary}
...
\subsection{Hyphen}\label{subsec:other-punct}
...
\subsection{Sentence Punctuation and Capitalisation}
...
\noindent Affixes, lemmas, etc. that cause lenition or nasalisation are marked in this grammar and in the dictionary
...
result. Generally, the former is used if the result is an abstract concept, e.g. \w{iý’ývî} ‘illuminate’ $\to$ \w{iý’ývîaû}
...
nouns \textit{do} exhibit natural gender, but this is only relevant for verbal inflexion.} and there are no
...
Table~\ref{tab:table-uf-declension} below shows the most common cases of the definite and indefinite declensions. Cases
are divided into primary (§~\ref{sec:decl-primary}), secondary (§~\ref{sec:decl-secondary}), and tertiary
cases (§~\ref{sec:decl-tertiary}). There is also an extended set of affixes only used in stacking; for that, see §~\ref{sec:declension-stacking}.
...
dialect.\footnote{They do in the \s{nd}, see §~\ref{sec:nd-nouns}.}
Table~\ref{tab:dale-declension} shows the paradigm of \w{ḍale} ‘table’ in its \s{def} and \s{indef} forms; this, of course,
...
\clearpage
\def\ufdeclensiontablestretch{1} % Set this to whatever is required to fill the page.
\begingroup
%\topskip-6pt
\let\arraystretch\ufdeclensiontablestretch
\null \vskip-4em
\vbox to 0pt {
\begin{table}[H]
\begin{center}
...
Ornative        & vé-\L            & véḍ-           && Ornative      & vé(t’hý)n-     & vét’hý-        \\ % mettre
\cline{1-3}\cline{5-7}
...
Ornative          & vét’halẹ         & véḍḍalẹ            && Ornative        & vénḍalẹ         & vét’hýḍalẹ    \\
\cline{1-3}\cline{5-7}
...
\end{tabular}
\caption{Paradigm of \w{ḍalẹ}.}\label{tab:dale-declension}
\end{center}
\end{table}
\vss
}
...
\section{Primary Cases}\label{sec:decl-primary}
...
an additional set of ‘augmented’ \s{abs} affixes; see §~\ref{sec:declension-stacking} for that.
...
see §~\ref{sec:declension-stacking} for that.
...
or belonging, there are also a number of verbs that govern the \w{gen}, and it also used to mark the standard of
...
noun, or after any adjectives. Possessive pronouns, too, typically precede the noun.
\begin{examples}
...
In comparisons, usually involving adjectives, the \s{gen} marks the standard of comparison (see §~\ref{subsec:comparison}).
\begin{examples}
...
\begin{examples}
    \item \w{Llac’hrírá \b{sý}árb.} ‘She wrote about trees.’
    \item \w{Jlí \b{abh}ráy’á Ahehár.} ‘I read of Caesar’s travels.’
    \item \w{Aúḅály’aû \b{á}rývýr.} ‘We were talking about history.’
\end{examples}
...
Note that the \s{insv} is \textit{not} used for appositions, but rather to introduce an unrelated noun. For appositions and
titles, see §~\ref{subsubsec:names-and-titles}. For the use of participles as adjectives, see §~\ref{subsec:participle-tense}.

\section{Secondary Cases}\label{sec:decl-secondary}
...
communicate the state of mind or material of something. See also §~\ref{subsec:ornative} on the \s{orn} case below,
...
\begin{examples}
    \item \w{Aú sdẹb’hní \b{saút’hŷn}árb.} ‘The man ceased being a tree.’
\end{examples}
...
\begin{examples}
    \item \w{Jlýaublit’héré \b{ré}vau.} ‘I defeated them with you.’
    \item \w{Lẹsẹhérélá lábhabh \b{réd’h}vẹ} ‘My father and I were arguing.’ (lit. ‘Father was arguing with me.’)
\end{examples}
...
\begin{examples}
    \item \w{Lẹvárc̣á Sehár B’héraû’z \b{b’he}lejy’aû.} ‘Caesar marched his legions to Rome.’
\end{examples}
...
\begin{examples}
    \item \w{Sý’b’héḷý jad’hé \b{lḷý}vau.} ‘I went there instead of you.’
\end{examples}

\subsection{Distributive}
The \s{distr} case can be translated as ‘per’ or ‘for each one’.
\begin{examples}
    \item \w{raút’h \b{cac’h}ly} ‘a loaf of bread for each of them’
    \item \w{réhèḍ \b{cahýn}db’hid’h} ‘income per capita’
    \item \w{b’hénvâ \b{cahýn}y’úr} ‘a daily occurrence’
\end{examples}

\subsection{Ornative}\label{subsec:ornative}
The \s{orn} case indicates that something possesses a quality or is endowed with something. Nouns in the
\s{orn} function much like adjectives, and in a sense, the \s{indef orn} can instead be thought of as a
derivational marker that turns nouns into adjectives and which functions much like English \textit{-ly}.
...
\begin{examples}
    \item \w{aú \b{vén}faúr} ‘the powerful man’ (lit. ‘the man endowed with force’)
    \item \w{sehár \b{vén}c’húraû} ‘a king bearing a crown’
\end{examples}
...
\s{indef}.
\begin{examples}
    \item \w{\b{vén}b’hauý’ý} ‘voluminous’ (lit. ‘endowed with volume’; a euphemism for ‘overweight’)
    \item \w{vár \b{vén}aúrd} ‘an orderly march’
\end{examples}
...
\begin{examples}
    \item \w{dáhŷnáẹ \b{ḅárýn}rê} ‘in a copper sky’
    \item \w{lvúr \b{ḅárý}vaût’há} ‘walls [which are] like mountains’
    \item \w{ḍalẹ \b{ḅárýn}rád’hyr} ‘the wooden table’
\end{examples}
...
\begin{examples}
    \item \w{abhár ec’hlérâ \b{vén}aúr} ‘a glimmering house decorated with gold’
    \item \w{abhár \b{vén}aúr ec’hlérâ} ‘a house decorated with glimmering gold’
    \item \w{abhár ec’hlérâ \b{ḅárýn}aúr} ‘a house shining like gold (but not actually made of gold)’
    \item \w{abhár \b{ḅárýn}aúr ec’hlérâ} ‘a glimmering house made entirely of gold’
\end{examples}

\section{Tertiary Cases}\label{sec:decl-tertiary}
...
\begin{examples}
    ...
    \item \w{\b{dwá}y’úr} ‘during the day’
...
\subsection{Adessive}
The \s{adess} case denotes positioning near or next to, but \textit{not} inside of something. As a result, it can also
be used to mean ‘outside’.
\begin{examples}
    \item \w{abhár \b{raú}ríb’hy’ér} ‘the house by the river’
\end{examples}
...
This case is one of the few places in the standard dialect where we can observe \w{áu} /ɑ̃u/ which is \textit{not} the
same as \w{aú} /ɔ̃/.
...
\begin{examples}
    \item \w{Vy’í sýrḅarḍ.} ‘I’m about to leave.’ (lit. ‘on the leaving’)
    \item \w{Sýrsẹ !} ‘[I’m] on it!’
\end{examples}
...
\begin{examples}
    \item \w{\b{dẹsŷ}y’éc’h dýsá} ‘two centuries ago’
    \item \w{\b{dẹh}ríb’hy’ér} ‘on this side of the river’
\end{examples}

\subsection{Transessive}
...
\begin{examples}
    \item \w{\b{ḅárŷ}y’éc’h dýsá} ‘two centuries later’
    \item \w{\b{ḅárŷ}y’úr sèḍ} ‘after seven days’
    \item \w{\b{ḅár}ríb’hy’ér} ‘across/beyond the river’
\end{examples}

Do not confuse this case with the \s{ess}, e.g. in the \s{indef pl}, we have
\s{transess} \w{ḅárŷn-} but \s{ess} \w{ḅárýn-}.

\subsection{Transallative}
The \s{transall} case denotes motion to the far side of or to beyond another object.
\begin{examples}
    \item \w{\b{Rébhár}árrih jráy’á.} ‘I travel (to) beyond the forest.’
\end{examples}

\subsection{Contressive}
The \s{contress} case denotes positioning opposite something.
\begin{examples}
    \item \w{abhár \b{át’h}árb} ‘the house opposite the tree’
\end{examples}

\subsection{Orientative}
The \s{orient} case denotes that an object is facing towards something.
\begin{examples}
    \item \w{\b{fér}saul} ‘facing the sun’
\end{examples}
...
\section{Names, Titles, and Appositions}\label{subsubsec:names-and-titles}
...
\begin{examples}
    \item \w{Daúvníc’h} ‘Dominic (\s{abs})’
    \item \w{Daúvníc’h} ‘Dominic (\s{nom})’
    \item \w{Cár} ‘Charles (\s{abs})’
    \item \w{Lác̣ár} ‘Charles (\s{nom})’
\end{examples}
...
same (proper) noun, \textit{both} are declined.
\begin{examples}
    \item \w{Snet’h C’haúfrér} ‘Brother Smyth’
    \item \w{Snet’h Ihaúfrér} ‘Brother Smyth (\s{acc})’
    \item \w{Snet’h Ihaúfrér Ihaj} ‘Brother Smyth the Wise (\s{acc})’
\end{examples}

Appositions also follow these rules, except that if they modify a common noun, they \textit{agree} with
the case and number of the noun instead of being declined \textit{instead} of it. Note that this is different
from modifier nouns, which take the \s{abs} instead. Consider:
\begin{examples}
    \item \w{ashasḍaúr, ast’haléraû véḍ} ‘to the beaver, a master carpenter’
    \item \w{ashasḍaúr, ast’haléraû, asvéḍ} ‘to the beaver, a carpenter, a master’
\end{examples}
...
\textit{is} an apposition and thus assumes \s{dat} case as well.

\section{Declension Stacking}\label{sec:declension-stacking}
...
referred to as \textit{declension stacking} or \textit{case stacking}. In many cases, what is actually intended to be declined
...
\begin{examples}
    \item \w{\b{B’hér-á-}c̣ár jrâhaút’hé.} ‘I met with Charles’ [family/children].’
    \item \w{Laráb’hérér \b{Lé-á-}hehár Lý’aúrýb’hihaú.} ‘Caesar’s [legions/soldiers] crossed the Rubicon.’
    \item \w{cabh \b{á-ḅárý-}vaût’há} ‘Giant’s Hat’\footnote{A species of mushroom.} (lit. ‘hat of the mountain-like one’)
\end{examples}

Cases can be stacked more than once, though this is rather rare.
\begin{examples}
    \item \w{Jráré \b{dẹ-á-vé-}húraû.} ‘I saw the royals.’ (lit. ‘the ones of the one bearing the crown’)
\end{examples}

Note that case affixes do \textit{not} agree with one another; rather, each affix individually agrees with whatever it
...
largely unmarked or only marked by mutations. Because of this, there is a special set of \s{abs} and \s{voc} affixes,
usually referred to as the \textit{augmented} \s{abs/voc}, that are only used in stacking:
\begin{center}
\begin{tabular}{l|>{\it}l|>{\it}lll|>{\it}l|>{\it}l}
Definite   &\nf Sg   &\nf Pl && Indefinite &\nf Sg &\nf Pl \\
\cline{1-3}\cline{5-7}
...
like true adjectives rather than verbs, i.e. whereas passive verb forms typically require the noun the modify to assume
...
\begin{examples}
    \item \w{váćár ḅérsâ} ‘piercing jaw’
    \item \w{Cár âdeb’haúrér} ‘Charles, the Devoured’
    \item \w{át’halẹ ḅẹt’hâ} ‘of the small table’
\end{examples}

\subsection{Tense of Adjectival Participles}\label{subsec:participle-tense}
...
\begin{examples}
    \item \w{Láráhó slẹlúrá b’héd’hẹhẹ dẹnájẹ.}  ‘The fish was too bulky to swim to the surface’\footnote{
        This is a very common proverb (often also just \textit{láráhó slẹlúr} ‘The fish is too bulky’)
...
Vocative      & /   & et’hè & /   & /                       & /     & evaú & /   \\
Genitive      & vaú & ḍaú   & \multicolumn{2}{l|}{ez/z’/’z} & naúḍ  & vaúd & lýr \\
...
\begin{examples}
    \item \w{jsylí} ‘I peruse it’
    \item \w{jsy\b{dy}lí} ‘I read from it’
    \item \w{lírá} ‘be perused!’
    \item \w{lí\b{dy}rá} ‘be read from!’
\end{examples}
...
\s{gen} pronoun is omitted:
\begin{examples}
    \item \w{Lábhabh ladvnéhá(\b{\%’z}) ib’his.} ‘The father reprimanded his son.’
    \item \w{Lẹraúhy’b’há(\b{*’z}) irá nés ádér.} ‘He raised his left arm up in the air.’
    \item \w{Svéc (\b{*z’})órdraûc llarét’hírá.} ‘She pulled a strand of hair from her head.’
\end{examples}
...
but unlike the normal personal pronouns, they \textit{do} have \s{nom}, \s{acc}, and \s{part} forms that are formed
...
\begin{examples}
    \item \w{Llẹlýad’hór.} ‘She loves herself.’
    \item \w{Llẹlýad’hór ilè.} ‘She loves herself very much.’
    \item \w{Llẹlýad’hór ilè dèl.} ‘She loves herself very much indeed.’
\end{examples}
...
\begin{examples}
    \item \w{Llẹlýad’hór.} ‘She loves her (someone else).’
    \item \w{Llẹlýad’hór ilè.} ‘She loves herself.’
    \item \w{Llẹvad’hór au ilè.} ‘She loves me and herself.’
\end{examples}
...
meaning; see~\ref{subsec:passive} for more about that.
...
\Paragraph{Gender}
UF verbs (and pronouns) inflect for gender in the \s{3sg} and sometimes in the \s{3pl}. This is \textit{not} grammatical gender,
...
\subsection{Passive}\label{subsec:passive}
...
that in a ‘passive’ clause, the verb \textit{does} take a passive affix even if there is an explicit object.
\begin{examples}
    \item \w{Y’ad’hór ivvâ.} ‘The mother is loved.’
    \item \w{Sylí dýliv́uhé.} ‘The book is being read.’
    \item \w{Dyyl syc’hahé.} ‘The window has been broken.’
\end{examples}
...
difference: the ‘active’ fundamentally corresponds to the \textit{agent}, and the ‘passive’ to the \textit{theme}. Thus, verbs
that only take a theme may translate as ‘active’ (or middle) in meaning despite being ‘passive’ in form:
\begin{examples}
    \item \w{Vv́ár dẹḅarḍ.} ‘I must leave.’
\end{examples}
...
Lastly, passive participles that are used as adjectives—like any other adjective—do \textit{not} govern any case and may
indeed appear next to even \s{nom} nouns (see §~\ref{sec:adjectives}).
...
are shown in Tables~\ref{tab:pres-ant-combined}~and~\ref{tab:pret-combined} below.
...
\s{3n}&aúsad’hór&aúlad’hór   &&\s{3n} &aúsyad’hór&aúlýad’hór\\\cline{1-3}\cline{5-7}
...
vary a lot more, but they are different depending on whether verb form following them starts with a vowel or a consonant.\footnote{This also
means that e.g. adding a consonantal passive prefix before a vocalic stem will change the future inflexion to be consonantal; compare
\w{b’had’hórérẹ́} ‘I will love’ as opposed to \w{jaúsyad’hórérệ} ‘I will love it’.}
...
\begin{enum}
\item in reported speech and indirect questions, e.g. \w{lladírá vad’hórhé} ‘she said she loved me’;
...
\item with certain adverbs, e.g. \w{ḅýt’hèḍ} ‘maybe’;
...
\textit{usually} a finite verb, but it may also be e.g. an infinitive if the future subjunctive of intent is nested
...
\begin{examples}
    \item \w{au árb au raû} ‘the tree and the log’
    \item \w{Lasẹhérélé au láb’haúré au láhaul.} \\ ‘The Sun and the North Wind were quarreling.’
    \item \w{Sav́áré lávaûd de au iraúl syl au sývaú vêâ.} \\ ‘The entire world had one language and the same words.’
\end{examples}
...
\w{fér} may be used instead, see §~\ref{subsubsec:personal-pronouns}—as there are no isolated personal pronouns in the
\s{nom}, \s{acc}, or \s{part}.
\begin{examples}
    \item \w{Llẹvad’hór au ilè.} ‘She loves me and herself.’
    \item \w{Llad’hór au ivvâ au ibhabh.} ‘She loves her mother and her father.’
    \item \w{Lýy’ad’hór au’z lávvâ au’z lábhabh.} ‘Her mother her father love her.’
    \item \w{Llẹvad’hór au y’fér.} ‘She loves me and her.’
    \item \w{Llẹy’ad’hór au vfér.} ‘She loves her and me.’
\end{examples}
...
\begin{examples}
    \item \w{Rab’haḍó iárb.} ‘We are felling the tree.’
    \item \w{Rab’haḍjô sárb.} ‘We fell trees.’
\end{examples}
...
\begin{examples}
    \item \w{Lẹrab’hat’há.} ‘He felled us.’
    \item \w{Llẹsyad’hór.} ‘She loves it.’
\end{examples}
...
is placed first.
\begin{examples}
    \item \w{B’hehýnác aúlýab’hat’hâ.} ‘With an axe, we have felled them.’
    \item \w{Iḷý dẹc’haúbhýrífá ḍẹv́ár.} ‘The place, you must have purified it.’
\end{examples}
...
\begin{examples}
    \item \w{Au lýr náý’acḍaúrâ sýec̣ asvaúr sýárb.} ‘And we shall indeed have revealed their sins to the world’\footnote{See
...
\section{Negated Clauses}\label{sec:negated-clauses}
...
and \s{aci}s/\s{pci}s see §§~\ref{subsec:subjunctive},~\ref{subsec:optative},~\ref{subsec:aci-pci}.
...
\begin{examples}
    \item \w{Aúlýab’hat’hâ’sý’ýâ b’hehýnác.} ‘We have not felled them with an axe.’
    \item \w{B’hehýnác asý’ýâ aúlýab’hat’hâ.} ‘It is not with an axe that we have felled them.’
\end{examples}
...
\begin{examples}
    \item \w{Ráv́â’sý’ýâ st’halẹ jac̣t'heá.} ‘Rarely have I ever bought a table.’
    \item \w{St’halẹ’sý’ýâ ráv́â jac̣t'heá.} ‘A table I have bought rarely.’
    \item \w{\#Ráv́â st’halẹ jac̣t'heá.} ‘(roughly) I rarely-bought a table.’
\end{examples}
...
\begin{examples}
    \item \w{*Ráv́â’sý’ýâ st’halẹ jac̣t'heá’sý’ýâ.} ‘(roughly) Rarely-not I bought a table.’
\end{examples}
...
first \textit{constituent}, not after the first word.} The main exception to this is with forms of \w{eḍ} ‘to be’,
which are typically immediately preceded by the question particle, the two forming a single word, placed at the very
end of the sentence:

\begin{examples}
    \item \w{St’halẹ \b{c’hes} jac̣t'heá ?} ‘Did I buy a table?’
    \item \w{Dwáḷýhes ilývy’ér ḍauḍéá \b{c’hes} ?} ‘Where did you get the light?’
    \item \w{Raúl baú \b{c’hes}se ?} ‘Is it a good language?’
\end{examples}
...
\textit{replaces} the question particle and unlike \w{c’hes} never merges with \w{eḍ} ‘to be’. It is also
possible to combine both particles.
\begin{examples}
    \item \w{St’halẹ c’hes jac̣t'heá’sý’ýâ ?} ‘Did I not buy a table?’
    \item \w{St’halẹvá jac̣t'heá ?} ‘I bought a table?’
    \item \w{St’halẹvá jac̣t'heá’sý’ýâ?} ‘I didn’t buy a table?’
    \item \w{Raúl baú c’hesse ?} ‘Is it a good language?’
    \item \w{Raúlvá baú se ?} ‘It is a good language?’
\end{examples}
...
\subsubsection{Fronting the Verb}
...
\begin{examples}
    \item \w{Jac̣t'heává st’halẹ ‽} ‘I \textit{bought} a \textit{table}?!’
    \item \w{Jac̣t'heá’sý’ýâvá st’halẹ ‽} ‘I \textit{didn’t} buy a \textit{table}?!’
\end{examples}

Note the order of particles:\footnote{See §~\ref{sec:particles} for more information on particle order.} negation
...
\subsubsection{Simplified Questions}
...
\begin{examples}
    \item \w{Ýrŷ c’hes ?} ‘Are you happy?’
    \item \w{Sýr c’hes ?} ‘Are you sure?’
    \item \w{C’hes ?} ‘Is it?’ / ‘Really?’
\end{examples}

Person marking is generally required if the addressee is not in the second person:
\begin{examples}
    \item \w{Sýr c’hes aúdẹvárc ?} ‘Are you sure that this will work?’
    \item \w{Ausýró c’hes aúdẹvárc ?} ‘Are \textit{we} sure that this will work?’
\end{examples}
...
wh-words do \textit{not} replace the question particle; in fact, they can also be used without the question particle in
which case they act more like relative pronouns.
\begin{items}
    \item All variants of the locative pronoun \w{ḷýhes} ‘where, whither, whence, \ldots’
    \item \w{a} ‘who, what, whom, whose’.
    \item \w{c’haúy’ê} ‘how much, how many’.
    \item \w{c’há} ‘when’.
    \item \w{b’hehráy’ê} ‘by what means’.
\end{items}

Conversely, there are a few wh-words that act more like particles and replace \w{c’hes} in questions:
\begin{items}
    \item \w{c’hèl} ‘which, which one’.
    \item \w{c’hrá} ‘why’.
\end{items}

\subsubsection{\w{a c’hes} — ‘who, what, whom’}\label{subsubsec:aches-who-what}
...
\begin{examples}
    \item \w{Sa c’hes ḍad’hór ?} ‘What do you love?’
    \item \w{Ia c’hes ḍad’hór ?} ‘Whom do you love?’
    \item \w{Asa c’hes sydír ?} ‘To whom is it said?’
    \item \w{Sý’ẹ a c’hesse ?} ‘What is that?’
    \item \w{Árb áa c’hesse ?} ‘Whose tree is it?’
\end{examples}
...
\begin{examples}
    \item \w{Ŷnalẹ a c’hes ḍad’hór ?} ‘What table do you love?’
    \item \w{Árb a c’hesse ?} ‘What tree is it?’
\end{examples}
...
\subsubsection{\w{c’haúy’ê} ‘how much, how many’}
...
\begin{examples}
    \item \w{C’haúy’ê sýná c’hes dauhybh seh dwá-ádrrá ?} ‘How long have you been living in this place?’
    \item \w{Aúłraúlaúrẹ c’haúy’ê sýná c’hes láhẹh ?} ‘How long will this go on?’
\end{examples}
...
that the position they occupy is generally that of the second \textit{word} in—or alternatively right after—the phrase
they modify. If multiple particles modify the same phrase, they are grouped together in that position.
\begin{examples}
    \item \w{Sa \b{c’hes} ḍad’hór?} ‘What do you love?’
    \item \w{Rívnél \b{ḍèl} rá leb’h.} ‘He was a big \textit{scoundrel}.’
    \item \w{Rívnél rá \b{ḍèl} leb’h.} ‘He was a \textit{big} scoundrel.’
    \item \w{St’halẹ \b{vé} jad’hór.} ‘But I love a table.’
    \item \w{U \b{vé} st’halẹ u sárb jad’hór.} ‘But I love a table or a tree.’
\end{examples}
...
\begin{examples}
    \item \w{Jlí\b{’sý’ýâ} \b{vé} \b{dývrê} iliv́uhé.} ‘But at least I don’t read the book.’
    \item \w{Jlí\b{’sý’ýâ} \b{dývrê} \b{vé} iliv́uhé.}
    \item \w{Iliv́uhé \b{vé} \b{dývrê} jlí\b{’sý’ýâ}.}
    \item *\w{Jlí \b{vé}\b{’sý’ýâ} \b{dývrê} iliv́uhé.}
\end{examples}
...
phrase’, as there are no prepositions in UF.\medskip
...
\end{forest}\par\medskip
A possible tree for \w{Jlí’sý’ýâ c’hes vé dývrê iliv́uhé ?} ‘But do I at least not read the book?’\par}\bigskip
...
\Bar{V} particles (of which there may be several, and those in turn \textit{can} again be
...
that govern the \s{part} or in some cases to indicate that an action is incomplete (see §~\ref{subsubsec:declension}).
\gloss {
...
Section~\ref{subsubsec:negated-optative}), attached to the verb of the \s{aci}. Note that the
matrix clause is negated normally. Where the meaning of the two is equivalent, negating
the \s{aci} is generally preferred.
\begin{examples}
    \item \w{Lácár sbhaú \b{t’h’}àfér láȷ́éd’há.} ‘Charles ordered that no bridge be built.’
    \item \w{Lácár sbhaú àfér láȷ́éd’há\b{’sý’ýâ}.} ‘Charles did not order a bridge to be built.’
\end{examples}
...
\begin{examples}
    \item \w{Lác̣ár dẹ\b{lý}ḅarḍ láȷ́éd’há.} ‘Charles ordered them to leave.’
    \item \w{Lác̣ár \b{àsy}fér láȷ́éd’há.} ‘Charles ordered it to be built.’
\end{examples}
...
\begin{examples}
    \item \w{Lác̣ár dẹlý\b{dy}ḅarḍ láȷ́éd’há.} ‘Charles ordered them to get going.’
\end{examples}


\section{Conditionals}\label{subsec:conditionals}
Traditional UF grammar divides conditionals into four categories: Simple (\s s), potential (\s p), counterfactual (\s c), and
...
\begin{examples}
    \item[(\s s)] \w{S r sré, aû-r sfe.} ‘If \w{r} is true, then not-\w{r} is false.’\footnote{UF does not
...
\begin{examples}
    \item \w{S sfúr snaûb, ád’hý rêrá sejú.} ‘If a number is provided, it shall be greater than two.’
\end{examples}
...
\begin{examples}
    \item[(\s p1)] \w{S ḍẹsẹhúrvé, aúrrzaúsḍressa júrdy’í.} \\ ‘If you were to help me, it could be finished today.’
    \item[(\s p2)] \w{S vê ḍẹy’ẹhẹhúrvé, aúrrzaúsḍressa aḍrdvê.} \\ ‘If you were to help me tomorrow, it could be finished the day after tomorrow.’
\end{examples}
...
instead:
\begin{examples}
    \item[(\s s)] \w{S ḍẹsẹhúrvé, aúrrzaúsḍre júrdy’í.} \\ ‘If you help me, it will (with certainty) be finished today.’
\end{examples}
...
\begin{examples}
    \item[(\s c)] \w{S ḍẹsẹhúsvé, aúrrzaúsḍressá.} \\ ‘If you were helping me, it would be finished.’
    \item[(\s c)] \w{S ḍẹsẹhúhávé, aúrrzaúsḍressá y’ér.} \\ ‘If you had helped me, it would have been finished yesterday.’
\end{examples}
...
\begin{examples}
    \item[(\s i1)] \w{S ḍẹy’ẹhẹhúrvé, srzaúst’há y’ér !} \\ ‘If only you were helping me—it would have been finished yesterday!’
    \item[(\s i2)] \w{S vê b’háy’ẹhẹhúrrevé, aúr-rzaúsẹre-śe aḅrdvê.} \\ ‘(roughly) If you had been able to help me tomorrow, it would have been finished the day after.’
\end{examples}
...
\begin{enum}
    \item ð > ð̞ [ð̺˕]
    \item z, ʑ > s, ʃ / \#\_, C[-voice]\_, \_C[-voice]
    \item ɕ, ȷ̊ > ʃ
    \item x > ʃ / V[+front]\_, \_V[+front]
    \item ɮ̃ > ɬ̃ > ʃ  / \_V
    \item ɰ > ː / V\_
    \item ɰ̃ > ɰ
    \item ʀ > x
...
\begin{enum}[9]
    \item ḅ > b / V\_
    \item b > β
    \item y, ỹ, ỹ̃ > ʉ, ʉ̃, ʉ̃̃
    \item e, ɛ > i̯ / V\_
    \item o > u / a\_, ã\_, ã̃\_
    \item o > ɔ
    \item C[+fricative] > ː / V\_\#
    \item h > $\emptyset$ / V\_V
\end{enum}
}
...
\w{aùḕ} /au̯ˈɛː/, though \w{x̄́} is written \w{x̌} instead, e.g. \w{aḅraúc̣} > \w{ab’hraǔ} /aˈβɰɔ̃ː/. A notable exception to this process
...
\section{Nouns}\label{sec:nd-nouns}
...
latter still apply to these prefixes, e.g. the \s{nom} affix \w{lá-\L} is \w{ṣ’há-\L} instead. Entries marked with \textit{/}
...
differences:
\begin{items}
    \item Such contractions are restricted to declensions only, e.g. \s{sd} \w{lá-áb’há} > \s{nd} \w{ṣ’hâuá}.
...
\noindent Βορέας καὶ Ἥλιος περὶ δυνάμεως ἤριζον. ἔδοξε δὲ αὐτοῖς ἐκείνῳ τὴν νίκην ἀπονεῖμαι, ὃς ἂν αὐτῶν ἄνθρωπον ὁδοιπόρον
...
\Paragraph{Translation by Émile Chambry}\smallskip
\noindent Borée et le Soleil contestaient de leur force. Ils décidèrent d’attribuer la palme à celui d’entre eux qui dépouillerait
...
\s{insv}-man|then|remove-\s{pres.ant.ptcp}|\s{acc}-clothing|supplementary|\s{def}\Sl sun|\s{3n}-throw-\s{pres.ant}|\s{acc.indef.pl}-ray
...
%% Dictionary.
\chapter{Dictionary}\label{chap:dictionary}
...
abbreviations, usually in italics. For verbs, \textit{tr.} and \textit{intr.} next to the part of speech may be
...
\begin{glossary}{3}{4em}
    ...
    \g{fem.}{feminine}
    \g{fig.}{figurative}
    \g{indecl.}{indeclinable\footnotemark}
    \g{interj.}{interjection}
    \g{interrog.}{interrogative}
    \g{intr.}{intransitive}
    \g{irreg.}{irregular}
    \g{lit.}{literally}
    \g{masc.}{masculine}
    ...
    \g{v.}{verb}
\end{glossary}
\footnotetext{Also used for things other than nouns to indicate that they are invariant.}
