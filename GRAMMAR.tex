...
Many of the case prefixes cause lenition in the initial consonant of the noun, e.g. *ḍalẹ* ‘table’ to
...
\noindent The table below shows the paradigm of *ḍale* ‘table’ in its definite and indefinite forms; this, of course,
is a rather extreme example, as the initial *ḍ* means that it is subject to both lenition and nasalisation.
...
\end{tabular}
\caption{Paradigm of *ḍalẹ*.}\label{tab:vocalic-declension}
\end{table}
...
‘X’ in ‘to be X’, e.g. *Aúsó ḍe ráhó* ‘We are all fish’.

Secondly, it turns nouns into modifier nouns, e.g.
*árḍihyl lývy’ér* ‘light particle (as in, a particle of light)’. In this second use, the modifier is generally
...
*Seb’h dwásẹ láraúb’hẹ* ‘The robot was here’. Apart from this, the \s{nom} isn’t really used in UF.

\subsubsection{Vocative}
The vocative is a rare case that is used to address someone or something, e.g. *C̣ár!* ‘O Charles!’. The vocative
only occurs in the definite and usually with proper nouns. Note that the vocative is not marked by an affix, but
rather by mutation only, e.g. *C̣ár* from *Cár*.
...
mark the direct object of a verb. While some verbs, e.g. *ub’hrá* ‘to be
...
*jlí sliv́uhé* ‘I peruse a book’ vs *jlí dŷnliv́uhé* ‘I read (\s{pres}) from a book’ or ‘I am reading a book’. Consequently,
\s{pres ant} forms, which are mainly perfective, generally take the \s{acc}, e.g. *jlíé iliv́uhé* ‘I’ve read the book’,
whereas \s{pret} forms, which are mainly imperfective, generally take the \s{part}, e.g. *jlíá dyliv́uhé* ‘I was reading
...
omitting the agent affix; the ‘subject’, if there is any, will then be in the \s{acc} or \s{part}, e.g. *sylí dýliv́uhé*
...
The \s{dat} case is used to indicate the indirect object of some verbs, *jdónẹ́ḷẹ* ‘I bestow upon him’. Note that some
...
Another use of the dative is to indicate a beneficiary or recipient in a more general sense, e.g. *jsyfér asvẹ* ‘I did
it for me/us’.

\subsubsection{Essive}
The \s{ess} case is used to mean ‘as X’ or ‘in the form of X’ or ‘in the role of X’, e.g. *lẹt’hiy’e dyt’halẹ ḅárýny’éjúré*
...
or movement towards or away from an object, e.g. *B’hét’hebhaú* ‘to France’.

The *d* in the \s{def abl sg} and is omitted if the noun starts with a consonant,
e.g. *rêḍalẹ* ‘from the table’; be careful especially with words that start with *s*, whose \s{abl sg} is often mistaken
for a plural, e.g. *rêsol* ‘from the floor’, but *rêssol* ‘from the floors’.
...
The \s{instr} case is used to indicate the instrument or means using or by which an action is performed, e.g. *b’heac*
‘with an axe’.

The *hý* in the \s{indef instr sg} may be omitted arbitrarily, particularly in literary language; there
...
The *n* in the \s{indef instr sg} is omitted according to the same rules as the *d* in the \s{def abl sg}.
...
Adjectives generally—but not always—follow the noun they modify and are never inflected, e.g. *át’halẹ ḅẹt’hâ* ‘of the small table’.
There is no established order of adjectives.

\subsubsection{Adverbs}
Adverbs are formed from adjectives in one of two ways. For adjectives that are not adjective verbs, *-(é)vâ* is added,
e.g. *c’haú* ‘holy’ becomes *c’haúvâ* ‘holily’; the *e* is only present if the adjective ends with a consonant. Adjectives that
are derived from adjective verbs instead replace the *-â* affix with *-vâ*, e.g. *réâ* ‘true’ becomes *révâ* ‘truly’. This
form *never* inserts an *e*.
...
*do* take active person marking even if there is an explicit subject e.g. *lávvâ llad’hór* ‘the mother loves’, they do
*not* take passive person marking if there is an explicit object (unless there is no subject), e.g. *lávvâ llvad’hór* ‘the
mother loves me’, vs *lávvâ llad’hór iáb’há* ‘the mother loves the child’, which has *llad’hór* ‘she loves’ instead of
*llsyad’hór* ‘she loves it’.
...
no true distinct syntactical or morphological passive, e.g. *y’ad’hór ivvâ* ‘the mother is loved’.\footnote{Note the accusative
...
actual writing, e.g. *ḍad’hór* ‘you love’ vs *dad’hór* ‘to love’; since the dot is omitted in writing, both forms
look the same: *dad’hór*. Moreover, the \s{2sg pass} is identical to the \s{2sg act} in any case.
...
*jvvaúríhe* ‘to remember’ but *jaúvvaúríźẹ́* ‘I shall remember’.
...
The future *subjunctive* uses a different stem; for that, see §~\ref{subsec:subjunctive}.
...
is used to express a hypothetical past, e.g. *jaúvvaúríźessệ* ‘I could have loved’. In reported speech, this can lead
...
The *ú* is omitted if the infix is followed by the vowel, in which case it causes nasalisation. The presence of the gnomic
...
The dative affixes *-vé* ‘me, us’, *-b’hẹ* ‘you’, and *-ḷẹ* ‘him, her, it, them’ are used in conjunction with
...
the rest of the verb, e.g. *dedónẹ́* ‘to bestow’ to *dedónẹ́ḷẹ* ‘to bestow upon him’.

These affixes are generally not used if the \s{dat} assumes the sense of ‘for someone’, or ‘to someone’; for instance, while
*fúr* ‘to provide’ takes a \s{dat} as its indirect object, e.g. *jfúrb’hẹ* ‘I provide you (with something)’, the verb
*fér* ‘to do, make’ does not, and thus, it is not e.g. \**jsyférvé*, but rather *jsyfér asvẹ* ‘I did it for me/us’, where
*asvè* is the \s{dat} inflexion of the \s{1sg} pronoun.
...
one over the other with certain verbs, and some verbs regularly admit both, albeit with different meanings, e.g. *jsydíréḷẹ*
‘I said it to him’ vs *jsydíré aslẹ* ‘I said it for his sake’.
...
stem of *vvaúríhe* is *vvaúríźe*, so the future subjunctive stem is *vvaúríźeśe*. The subjunctive stem coalesces like
...
The main semantic difference between this construction and *b’he* is that the latter strictly means ‘in order to’ or ‘so that’,
...
by prefixing *y’(ẹ)\L* to the indicative (or future) stem,\footnote{The use of the (future) subjunctive stem to form the optative, with
no change in overall meaning, is fairly archaic and only encountered in poetry in modern UF.} e.g. *dẹvy’ẹvvaúríhe* ‘may
you remember me’. As ever, the *(ẹ)* is omitted if the stem starts with a vowel.

In the future, this generally does *not* change whether the consonantal or vocalic affixes are used! If the stem was
...
*ý’*, which this is dropped in the optative: e.g. *náý’ad’hóraú* ‘we shall love’ becomes *náy’ad’hóraú* ‘may we love’ (the
difference is minor: *ý’* vs *y’*). A bare optative is difficult to translate into English; a more precise explanation of what
...
actual meaning of *dẹvy’ẹvvaúríhe* is roughly ‘I hope that you remember me’.\footnote{The context of this could be e.g.
...
certain verbs, e.g. *ub’hrá* ‘can, may, might’, also have constructions with the optative.
...
of *eḍ* and its present tense,\footnote{The original morphological preterite anterior tense of \textit{eḍ} was lost in
Late Middle UF.} e.g. *t’hẹdâ vy’í* ‘I had been’. Note that only the participle is
inflected for mood in this case, e.g. subjunctive *t’hẹrâ vy’í* ‘I should have been’.
...
of the corresponding passive affix—or before the suffix part if there is no prefix part—e.g. *jsylí* ‘I peruse it’ vs
*jsydylí* ‘I read from it’ or *lírá* ‘be perused!’ vs *lídyrá* ‘be read from!’.\footnote{The only passive forms that do
...
remember a table of his/hers/its’. If the following word starts with a vowel, it is somtimes written *z’*,\footnote{The
...
*t’hé*, which becomes *t’h’* before vowels.} e.g. *ḍevvaúríhe z’it’halẹ* ‘to remember his/her/its table’, though
*ḍevvaúríhe’z it’halẹ* is also common and preferred in traditional literature.
...
indefinite case when it refers to a thing, e.g. *Sa c’hes ḍad’hór?* ‘What do you love?’, and definite
case when it refers to a person, e.g. *Ia c’hes ḍad’hór?* ‘Whom do you love?’
...
*jsysav́á* ‘I know it’. It *would* be possible to add the object marker in the example above, but it would
...
is added regardless, seeing as the verb would not be finite otherwise, e.g. *sysav́á* ‘it is known that’.
...
*-dy-* (see §~\ref{subsubsec:personal-pronouns})
\gloss {
...
This sentence indicates that the speaker believes that, if the addressee helps them, there is a *possibility* that they
