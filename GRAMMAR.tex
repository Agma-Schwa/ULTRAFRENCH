...
UF nouns can be declined more than once by taking an inflected noun form and declining it again. Generally, only a
select few cases, particularly the \s{gen}, \s{orn}, and \s{ess}, are eligible for this. This is referred to as \textit{declension
stacking}. In many cases, what is actually intended to be declined is an implied ‘the one(s)’, or a noun that is omitted
and obvious from context. The hyphens used here are only for clarity; they are omitted in actual UF writing.
\begin{examples}
