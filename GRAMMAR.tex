...
nasal vowel, e.g. *vvaúríhe* ‘to remember’ forms *vvaúríhê* ‘remembering’, and *ad’hór* forms *âd’hór* ‘being loved’.
...
\gloss {
    Rab’hadó iárb.
    r-ab’haḍ-ó| i-árb
    \s{1pl.act}-fell-\s{1pl.act}| \s{acc.def.sg}-tree
...
\gloss {
    Rab’hadjô sárb.
    r-ab’haḍ-jô| s-árb
    \s{1pl.act}-fell-\s{gnomic}\Sl\s{1pl.act}| \s{acc.indef.pl}-tree
    ‘We fell trees.’
}
...
\gloss {
    Lerab’hat’há.
    lẹ-r-ab’ha\Sl t’há.
    \s{3sgm.act}-\s{1pl.pass}-fell\Sl\s{3sg.pres.ant}
    ‘He felled us.’
}
...
\gloss {
    ...
    \s{instr.sg.indef}-axe|\s{1pl.act}-\s{3pl.pass}-fell\Sl\s{1pl.pres.ant}
...
\gloss {
    ...
    And|their|\s{1pl.act.fut.ant}-cleave-\s{circ}|\s{acc.pl.def}-sin|\s{dat.sg.def}-world|\s{acc.pl.def}-tree
...
Negation is expressed using the particle *asý’ýâ* ‘not’, which is often appended to verbs as *’sý’ýâ*.
By default, the particle is placed right after the verb:
\gloss {
    Aúlýab’hat’hâ’sý’ýâ b’hehýnác.
    aú-lý-ab’ha\Sl t’hâ|’sý’ýâ|b’hehýn-ác.
    \s{1pl.act}-\s{3pl.pass}-fell\Sl\s{1pl.pres.ant}|not|\s{instr.sg.indef}-axe
...
\gloss {
    ...
    \s{instr.sg.indef}-axe|not|\s{1pl.act}-\s{3pl.pass}-fell\Sl\s{1pl.pres.ant}
...
\gloss {
    ...
    \s{instr.sg.indef}-axe|\s{1pl.act}-\s{3pl.pass}-fell\Sl\s{1pl.pres.ant}|not
...
\gloss {
    ...
    seldom|not |\s{acc.sg.indef}\Sl table|\s{1sg.act}-buy\Sl\s{3sg.pres.ant}
...
\gloss {
    ...
    seldom|\s{acc.sg.indef}\Sl table|\s{1sg.act}-buy\Sl\s{3sg.pres.ant}
...
\gloss {
    ...
    \s{acc.sg.indef}\Sl table|not| seldom |\s{1sg.act}-buy\Sl\s{3sg.pres.ant}
...
\gloss {
    ...
    seldom|not |\s{acc.sg.indef}\Sl table|\s{1sg.act}-buy\Sl\s{3sg.pres.ant}|not
...
\gloss {
    ...
    \s{acc.sg.indef}\Sl table|Q|\s{1sg.act}-buy\Sl\s{3sg.pres.ant}
    ‘Did I buy a table?’
}
...
\gloss {
    ...
    \s{acc.sg.indef}\Sl table|Q|\s{1sg.act}-buy\Sl\s{3sg.pres.ant}|not
...
\gloss {
    ...
    \s{acc.sg.indef}\Sl table|not|Q|\s{1sg.act}-buy\Sl\s{3sg.pres.ant}
...
%% NOTE: There are 3 kinds of comparatives in UF:
%% - Affirming comparative: ‘better and also good’
%% - Neutral comparative: ‘better’
%% - Denying comparative: ‘better but not good’
...
\gloss {
    Çárvá, sráhó dwávaût’há daçt’heá?
...
\gloss {
    ...
    \s{cons.pl.def}-law|all|well.known|\s{gen.sg.def}\Sl aviation|way|there.is.no|\s{inf.act-subj}\Sl can
}

\gloss {
    dy-b’hóy’ẹ | s-ab’héy’ |ez |lé-el | la-lẹ-bet’hẹ|z’
    \s{part.sg.def}-to.fly|\s{acc.sg.indef}-bee|its|\s{nom.pl.def}-wing|\s{3pl.act-comp}-be.small|its
}

\gloss {
    ihór | bet’hê | rê-sol | ḍ-auḍé
    \s{acc.sg.def}\Sl body| be.small\Sl\s{part.act}|\s{abl.sg.def}-soil|\s{inf.act}-obtain
}
...
%% turn a noun into a verb that roughly means ‘to use that thing’. E.g. aç ‘axe’ -> açt’he  ‘roughly: to use
%% an axe’.
