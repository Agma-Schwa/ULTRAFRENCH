...
#let dictionary(file, plugin-path, lemma-format) = {
    let dictionary-plugin = plugin(plugin-path)
    let dictionary-file = read(file)
    let dictionary-obj = json(dictionary-plugin.generate_dictionary(bytes(dictionary-file)))
    let typeset-entry(entry) = {
    ...
    dictionary-obj.entries.map(typeset-entry).join()
}
